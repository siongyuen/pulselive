...
### Added
- **`pulsetel health` command** - Show overall health score (0-100) with critical/warning counts and status
- **`pulsetel webhooks` command** - List and test configured webhooks
- **`pulsetel sentry` command** - Run Sentry checks standalone

### Fixed
- **Corrupt package.json handling** - Deps check now validates JSON syntax and warns instead of silently failing
- **Reporter crash** - Fixed crash when deps check returns warning with non-standard details structure

## [2.1.2] - 2026-04-20

### Fixed
- **Critical: npm audit severity mapping bug** — npm uses 'moderate' for medium-severity vulnerabilities, but PulseTel expected 'medium'. Moderate vulnerabilities were silently dropped from counts, causing underreporting of security issues. Added severity mapping to correctly translate npm severity names to internal levels.
